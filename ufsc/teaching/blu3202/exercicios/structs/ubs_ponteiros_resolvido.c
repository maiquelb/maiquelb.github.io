...
*/
Ubs* ubs_mais_proximas(Ubs ubs[]){
   Ubs* result = malloc(sizeof(Ubs)*2);
...
int main() {
    ...
    while(opcao!=9){
       ...
       if(opcao==3){   
          Ubs *pubs = ubs_mais_proximas(ubs);
          printf("UBSs mais próximas: (%s-%s), (%s-%s), \n", pubs[0].cnes, pubs[0].nome, pubs[1].cnes, pubs[1].nome);
       }
    }
